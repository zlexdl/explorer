        ...
        if (block) {
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              var nvin_address = new Array()
              var nvout_address = new Array()
              lib.syncLoop(vin.length, function (loop) {
                var i = loop.iteration();
                update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', function(){
                  
                  redis.getHolderName(nvin[i].addresses, function(res) {
                    nvin_address.push(res);
                    loop.next();
                  })
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, txid, vout[t].amount, 'vout', function(){
                      redis.getHolderName(vout[t].addresses, function(res) {
                        nvout_address.push(res);
                        subloop.next();
                      })
                    });
                  } else {
                  ...
                  lib.calculate_total(vout, function(total){
                    ...
                    if (total/100000000 > 500) {
                      mailOptions.subject = settings.coin + ':' + total/100000000;
                      var html = 'http://explorer.nebl.io/tx/' + tx.txid + '<br>';
                      html = html + '<table border="1"><tr><th>IN<th></tr>'
                      nvin_address.forEach(element => {
                        html = html + '<tr><td>' + element + '</td></tr>'; 
                      });
                      html = html + '</table><br>'
                      html = html + '<table border="1"><tr><th>OUT<th></tr>'
                      nvout_address.forEach(element => {
                        html = html + '<tr><td>' + element + '</td></tr>'; 
                      });
                      html = html + '</table>'
                      mailOptions.html = html ;
                      console.log("html=" + html);
                      sentmail.sendMail(mailOptions);
                    }
